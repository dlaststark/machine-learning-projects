...
def cnn_model(input_shape):
    ...
    # Convolution Layers
    x = SeparableConv2D(filters=16, kernel_size=(5, 5),
               padding='same', activation='relu', 
               kernel_initializer='he_normal', 
               kernel_regularizer=l1_l2(l1=0.1, l2=0.1), 
               name='CONV-1A')(x)
    x = BatchNormalization(axis=-1, name='BN_CONV-1A')(x)
    x = SeparableConv2D(filters=16, kernel_size=(5, 5), strides=(2, 2),
               padding='same', activation='relu', 
               kernel_initializer='he_normal', 
               kernel_regularizer=l1_l2(l1=0.1, l2=0.1), 
               name='CONV-1B')(x)
    ...
    x = SeparableConv2D(filters=32, kernel_size=(3, 3), 
               padding='same', activation='relu', 
               kernel_initializer='he_normal', 
               kernel_regularizer=l1_l2(l1=0.1, l2=0.1), 
               name='CONV-2A')(x)
    ...
    x = SeparableConv2D(filters=256, kernel_size=(3, 3), 
               ...
               kernel_regularizer=l1_l2(l1=0.1, l2=0.1), 
               name='CONV-5C')(x)
    x = BatchNormalization(axis=-1, name='BN_CONV-5C')(x)
    x = SeparableConv2D(filters=256, kernel_size=(3, 3), 
               padding='same', activation='relu', 
               kernel_initializer='he_normal', 
               kernel_regularizer=l1_l2(l1=0.1, l2=0.1), 
               name='CONV-5D')(x)
    x = BatchNormalization(axis=-1, name='BN_CONV-5D')(x)
    x = MaxPooling2D(pool_size=(2, 2), name='MAXPOOL-5')(x)
    x = Dropout(rate=0.25, name='DROPOUT_CONV-5')(x)
    
    # Fully Connected Layers
    x = SeparableConv2D(filters=512, kernel_size=(1, 1), 
               padding='same', activation='relu', 
               kernel_initializer='he_normal', 
               kernel_regularizer=l1_l2(l1=0.1, l2=0.1), 
               name='FC-1')(x)
    ...
    x = Dropout(rate=0.25, name='DROPOUT_FC-4')(x)
