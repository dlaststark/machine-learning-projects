...
from tensorflow.keras.layers import ZeroPadding2D, GlobalMaxPooling2D
...
def cnn_model(input_shape):
    ...
    # Stage 2
    x = convolutional_block(x, f=3, filters=[64, 64, 64], stage=2, block='A')
    ...
    x = convolutional_block(x, f=3, filters=[128, 128, 128], 
                            stage=3, block='A', dr=0.2, lr=0.2)
    x = identity_block(x, 3, [128, 128, 128], stage=3, 
                       block='B', dr=0.2, lr=0.2)
    x = identity_block(x, 3, [128, 128, 128], stage=3, 
                       block='C', dr=0.2, lr=0.2)
    x = identity_block(x, 3, [128, 128, 128], stage=3, 
                       block='D', dr=0.2, lr=0.2)
    
    # Stage 4
    x = convolutional_block(x, f=3, filters=[256, 256, 256], 
                            stage=4, block='A', dr=0.25, lr=0.25)
    x = identity_block(x, 3, [256, 256, 256], stage=4, 
                       block='B', dr=0.25, lr=0.25)
    x = identity_block(x, 3, [256, 256, 256], stage=4, 
                       block='C', dr=0.25, lr=0.25)
    x = identity_block(x, 3, [256, 256, 256], stage=4, 
                       block='D', dr=0.25, lr=0.25)
    
    # Stage 5
    x = convolutional_block(x, f=3, filters=[512, 512, 512], 
                            stage=5, block='A', dr=0.25, lr=0.25)
    x = identity_block(x, 3, [512, 512, 512], stage=5, 
                       block='B', dr=0.25, lr=0.25)
    x = identity_block(x, 3, [512, 512, 512], stage=5, 
                       block='C', dr=0.25, lr=0.25)
    x = identity_block(x, 3, [512, 512, 512], stage=5, 
                       block='D', dr=0.25, lr=0.25)
    x = identity_block(x, 3, [512, 512, 512], stage=5, 
                       block='E', dr=0.25, lr=0.25)
    
    # Stage 6
    x = convolutional_block(x, f=3, filters=[1024, 1024, 1024], 
                            stage=6, block='A', dr=0.25, lr=0.25)
    x = identity_block(x, 3, [1024, 1024, 1024], stage=6, 
                       block='B', dr=0.25, lr=0.25)
    x = identity_block(x, 3, [1024, 1024, 1024], stage=6, 
                       block='C', dr=0.25, lr=0.25)
    x = identity_block(x, 3, [1024, 1024, 1024], stage=6, 
