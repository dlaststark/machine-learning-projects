...
from tensorflow.keras.layers import Activation, LeakyReLU, Dropout
...
def identity_block(x, f, filters, stage, block, dr=0.1, lr=0.1):
    ...
    # Main Path
    x = Conv2D(filters=F1, kernel_size=(f, f), padding='same', 
...
def convolutional_block(x, f, filters, stage, block, s=2, p=2, dr=0.1, lr=0.1):
    ...
    # Shortcut Path
    x_shortcut = Conv2D(filters=F2, kernel_size=(f, f), padding='same', 
...
def cnn_model(input_shape):
    ...
    # Stage 7
    x = Conv2D(filters=1024, kernel_size=(3, 3), padding='valid', 
