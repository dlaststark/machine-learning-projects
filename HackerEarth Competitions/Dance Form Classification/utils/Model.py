...
def cnn_model(input_shape):
    ...
    # Stage 2
    x = magic_block(x, f=3, filters=[32, 64, 64], stage=2, block='A', dm=[6, 4, 2])
    x = identity_block(x, 3, [32, 64, 64], stage=2, block='B', dm=[6, 4, 2])
    x = identity_block(x, 3, [32, 64, 64], stage=2, block='C', dm=[6, 4, 2])
    x = identity_block(x, 3, [32, 64, 64], stage=2, block='D', dm=[6, 4, 2])
    
    # Stage 3
    x = magic_block(x, f=3, filters=[256, 128, 128], stage=3, block='A', dm=[2, 4, 6])
    x = identity_block(x, 3, [256, 128, 128], stage=3, block='B', dm=[2, 4, 6])
    x = identity_block(x, 3, [256, 128, 128], stage=3, block='C', dm=[2, 4, 6])
    x = identity_block(x, 3, [256, 128, 128], stage=3, block='D', dm=[2, 4, 6])
    
    # Stage 4
    x = magic_block(x, f=3, filters=[128, 256, 256], stage=4, block='A', dm=[6, 4, 2])
    x = identity_block(x, 3, [128, 256, 256], stage=4, block='B', dm=[6, 4, 2])
    x = identity_block(x, 3, [128, 256, 256], stage=4, block='C', dm=[6, 4, 2])
    x = identity_block(x, 3, [128, 256, 256], stage=4, block='D', dm=[6, 4, 2])
    x = identity_block(x, 3, [128, 256, 256], stage=4, block='E', dm=[6, 4, 2])
    
    # Stage 5
    x = magic_block(x, f=3, filters=[1024, 512, 512], stage=5, block='A', dm=[8, 6, 4])
    x = identity_block(x, 3, [1024, 512, 512], stage=5, block='B', dm=[8, 6, 4])
    x = identity_block(x, 3, [1024, 512, 512], stage=5, block='C', dm=[8, 6, 4])
    x = identity_block(x, 3, [1024, 512, 512], stage=5, block='D', dm=[8, 6, 4])
    x = identity_block(x, 3, [1024, 512, 512], stage=5, block='E', dm=[8, 6, 4])
    
    # Stage 6
    x = magic_block(x, f=3, filters=[512, 1024, 1024], stage=6, block='A', dm=[4, 6, 8])
    x = identity_block(x, 3, [512, 1024, 1024], stage=6, block='B', dm=[4, 6, 8])
    x = identity_block(x, 3, [512, 1024, 1024], stage=6, block='C', dm=[4, 6, 8])
    x = identity_block(x, 3, [512, 1024, 1024], stage=6, block='D', dm=[4, 6, 8])
    
    # Stage 7
    x = SeparableConv2D(filters=2048, kernel_size=(1, 1), padding='same', 
                        name='CONV-7', depth_multiplier=8, 
                        kernel_initializer='he_normal', 
                        kernel_regularizer=l1_l2(l1=0.05, l2=0.05))(x)
    ...
    x = SeparableConv2D(filters=4096, kernel_size=(1, 1), padding='same', 
                        name='CONV-8', depth_multiplier=8, 
                        kernel_initializer='he_normal', 
                        kernel_regularizer=l1_l2(l1=0.05, l2=0.05))(x)
    x = BatchNormalization(axis=-1, name='BN_CONV-8')(x)
    x = Activation('selu')(x)
    x = Dropout(rate=0.25, name='DROPOUT_CONV-8')(x)
