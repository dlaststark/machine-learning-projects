...
def prog_lang_detect_model(input_shape, output_shape):
    ...
    x = convolution_block(x, f=3, filters=[16, 64, 64])
    x = identity_block(x, f=3, filters=[16, 64, 64])
    x = identity_block(x, f=3, filters=[16, 64, 64])
    ...
    x = convolution_block(x, f=3, filters=[32, 128, 128])
    x = identity_block(x, f=3, filters=[32, 128, 128])
    x = identity_block(x, f=3, filters=[32, 128, 128])
    ...
    x = WeightNormalization(
          Dense(units=1024, activation='swish', kernel_regularizer=l2(0.0005), 
                kernel_initializer=he_uniform(seed=1)))(x)
    x = BatchNormalization()(x)
