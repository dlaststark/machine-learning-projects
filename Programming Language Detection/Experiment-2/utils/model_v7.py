...
def prog_lang_detect_model(input_shape, output_shape):
    ...
    x = convolution_block(x, f=3, filters=[16, 16])
    x = identity_block(x, f=3, filters=[16, 16])
    x = identity_block(x, f=3, filters=[16, 16])
    
    x = convolution_block(x, f=3, filters=[32, 32])
    x = identity_block(x, f=3, filters=[32, 32])
    x = identity_block(x, f=3, filters=[32, 32])
    
    x = convolution_block(x, f=3, filters=[64, 64])
    x = identity_block(x, f=3, filters=[64, 64])
    x = identity_block(x, f=3, filters=[64, 64])
    
    x = Conv1D(filters=128, kernel_size=3, strides=2, padding='same', 
               activation='swish', kernel_regularizer=l2(0.001),
               kernel_initializer=he_uniform(seed=1))(x)
    x = BatchNormalization()(x)
    x = AveragePooling1D(pool_size=2)(x)
